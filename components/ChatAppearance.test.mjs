...
const widthVariable = /var\(--chat-content-max-width, 1230px\)/g;

test("chat content keeps the existing 1230px default behind one shared variable", () => {
  assert.equal((chatWindow.match(widthVariable) ?? []).length, 2);
  assert.equal((chatInput.match(widthVariable) ?? []).length, 1);
  assert.match(globals, /--chat-content-max-width: 1230px;/);
